...
#define MAX_CLIENTS 10 // Maximum clients for extra credit threading
...
int exec_client_requests(int cli_socket)
{
    ...
    while (1)
    {
        ...
        // Handle "exit" command (client disconnects)
        if (strcmp(io_buff, "exit") == 0)
            break;

        // Handle "stop-server" command (shutdown server)
        ...
        if (strncmp(io_buff, "cd ", 3) == 0)
        {
            ...
            if (chdir(path) == 0)
            {
                send_message_string(cli_socket, "Changed directory\n");
            }
            else
            {
        ...
        // Process command as external command
...
int send_message_string(int cli_socket, char *buff)
{
    ...
    return send(cli_socket, buff, len, 0) == len ? OK : ERR_RDSH_COMMUNICATION;
}
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    for (int i = 0; i < clist->num; i++)
    {
        ...
        {
            // If not first command, set up pipe for stdin
            if (i > 0)
            {
                dup2(pipes[i - 1][0], STDIN_FILENO);
            }

            // If not last command, set up pipe for stdout
            if (i < clist->num - 1)
            {
            ...
            {
                // Redirect final command's stdout and stderr to client socket
    ...
    char exit_msg[32];
    snprintf(exit_msg, sizeof(exit_msg), "Pipeline exit code: %d\n", exit_code);
    send_message_string(cli_sock, exit_msg);
